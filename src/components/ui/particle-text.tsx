...
export const ParticleText: React.FC<ParticleTextProps> = ({
  ...
  className = '',
  primaryColor = [0.01, 0.41, 0.63], // #3b82f6
  ...
  const handleClick = useCallback((e: React.MouseEvent) => {
    ...
    for (let i = 0; i < explosionParticles; i++) {
      ...
      const velocity = 2 + Math.random() * 30;
      particlesRef.current.push({
        x: mouse.x,
        y: mouse.y,
        targetX: mouse.x,
  ...
  return (
    <div className={`relative w-full min-h-[400px] overflow-hidden ${className}`}>
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full opacity-0 pointer-events-none" />
      <canvas ref={webglCanvasRef} className="absolute inset-0 w-full h-full" onMouseMove={handleMouseMove} onClick={handleClick} />
    </div>
  );
};
